...
from winusbcdc import (DIGCF_DEVICE_INTERFACE,
                       DIGCF_PRESENT, ERROR_IO_INCOMPLETE,
...
class ModiWinUsb(object):
    ...
    def __init__(self):
        ...
        self._index = -1
        self.vid = [0x2FDE, 0x2FDE]
        self.pid = [0x0001, 0x0002]
    ...
    def find_device(self, path):
        return is_device(None, self.vid[0], self.pid[0], path) or is_device(None, self.vid[1], self.pid[1], path)
...
class ModiWinUsbComPort:
    ...
    def readinto(self, buf):
        ...
        if self._rxremaining:
            len_rxremaining = len(self._rxremaining)
            read = min(len_rxremaining, orig_size)
