    ...
    def update_module_firmware(self, modi_ports):
        ...
        while True:
            ...
            for index, stm32_updater in enumerate(self.stm32_updaters):
                if self.state[index] == 0:
                    ...
                    if self.wait_timeout[index] > 3:
                        ...
                        if stm32_updater.network_uuid:
                            ...
                            stm32_updater.update_error_message = "No response network module"

                elif self.state[index] == 1:
                    # get module update list (only module update)
                    is_done = False
                    if stm32_updater.update_in_progress:
                        self.num_to_update[index] = len(stm32_updater.modules_to_update) + 1
                        self.state[index] = 2
                    else:
                        self.wait_timeout[index] += delay
                    ...
                    if stm32_updater.update_error == 1:
                        ...
                        if self.list_ui:
                            ...
                            self.list_ui.set_error_message(index, stm32_updater.update_error_message)
                ...
                elif self.state[index] == 100:
                    #reconnect
                    is_done = False
                    if not stm32_updater.popup_reconnect:
                        self.wait_timeout[index] += delay
                        if self.list_ui:
                            self.list_ui.network_uuid_signal.emit(index, "reconnected")
                            self.list_ui.network_state_signal.emit(index, 0)
                        ...
                        if self.wait_timeout[index] > 5:
                            ...
                            if stm32_updater.network_uuid:
                                ...
                                self.state[index] = 3
                                stm32_updater.update_error = -1
                                stm32_updater.update_error_message = "Not response network uuid"
